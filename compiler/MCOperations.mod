...
*)
    ...
    BEGIN
      ...
      ELSIF tp = boolptr THEN
        IF op = orsy THEN
          v3 := VAL(CARDINAL, VAL(BOOLEAN, v1) OR VAL(BOOLEAN, v2));
        END;
    ...
    BEGIN
      ...
      ELSIF tp = boolptr THEN
        IF op = andsy THEN
          v3 := VAL(CARDINAL, VAL(BOOLEAN, v1) AND VAL(BOOLEAN, v2));
        END;
      ELSIF tp^.form = sets THEN
        IF op = times THEN
    ...
    BEGIN
      res.value := VAL(CARDINAL, NOT VAL(BOOLEAN, c1.value));
    END NotOp;
