...
FROM MCBase IMPORT
  ...
  Constval, Symbol, intptr, intcarptr, noprio;
FROM MCP3IO IMPORT
...
MODULE ExpressionSystem;
  ...
  IMPORT
    scalars, oneword, doubleword, maxint, maxcard,
    QualIdent, MCBase, TypeHandling;
  ...
  BEGIN
    IF strval^.loadoffset = maxcard THEN
  ...
  BEGIN
    ...
    PutWord(sp);
    PutWord(cval.value);
    IF TypeHandling.IsString(sp) THEN
...
*)
           ...
           |4: Expression(at);
               WITH at DO
                 IF NOT ((mode = varm) AND (atp = processptr)) THEN
                   ErrorLS(127)
                 END
               END;
               EXIT;
  ...
  BEGIN
    WITH x^ DO
      TypeHandling.OpenTest(idtyp);
        IF klass IN Idset{vars,fields} THEN at.mode := varm;
        ELSE at.mode := exprm;
        END;
        at.atp := idtyp;
  ...
  BEGIN
    REPEAT
      IF sy = lbrack THEN PutGetSy;
        Expression(lat);
          IF at.atp <> NIL THEN
              IF at.atp^.form = arrays THEN
                TypeHandling.OpenTest(at.atp^.ixp);
                TypeHandling.OpenTest(at.atp^.elp);
                indat.atp := at.atp^.ixp; indat.mode := varm;
(*
...
*)
      ...
      ELSIF sy = period THEN
        PutGetSy;
          IF at.atp <> NIL THEN
              IF at.atp^.form <> records THEN Error(131)
              ELSE
                Locate(at.atp^.fieldp,x); (* entries with klass = fields assumed *)
                IF x <> NIL THEN
                  PutSy(namesy); PutWord(x);
                  TypeHandling.OpenTest(x^.idtyp);
                  at.atp := x^.idtyp;
                ELSE Error(73)
                END;
              END
        ...
        GetSy;
      ELSIF sy = arrow THEN
          IF at.atp = addrptr THEN at.atp := wordptr
          ELSIF at.atp <> NIL THEN
              IF at.atp^.form <> pointers THEN Error(146)
              ELSE
                TypeHandling.OpenTest(at.atp^.elemp);
                at.atp := at.atp^.elemp
              END
          END;
        PutGetSy;
          ...
          IF atp <> NIL THEN
            WITH ls^ DO
              ...
              ELSIF idtyp = wordptr THEN
                IF NOT TypeHandling.WordComp(atp) THEN ErrorLS(150) END;
              ELSE
    ...
    PROCEDURE SimpleExpression(VAR at : Attribut);
      ...
      PROCEDURE Term(VAR at : Attribut);
          ...
          BEGIN (* SetConstructor *)
            ...
            WHILE sy <> rconbr DO
              ...
              IF (styp <> NIL) AND
                ...
                IF (v1 < smin) OR (v2 > smax) OR (ctyp = intptr) THEN ErrorLS(98);
                ELSE
                  spat := spat + RangeSet(v1,v2);
                END;
              ELSE ErrorLS(97);
              END;
        ...
        BEGIN (* Factor *)
              ...
              ELSE
                ...
                WITH x^ DO
                  CASE klass OF
                   ...
                   |types :
                      CASE sy OF
                        lconbr: SetConstructor(at); mustreset := FALSE;
                       |lparent:
                          PutSy(namesy); PutWord(x);
                          TypFunction(at);
                       |lbrack, period, arrow:
          ...
          END;
          IF at.mode = constm THEN
            IF mustreset THEN ResetSave(save) END;
            ...
            IF ExprComp(at1,at2) THEN
              ...
              IF atp = intptr THEN
                WHILE INTEGER(c1.value) < INTEGER(c2.value) DO
                  IF c1.value = maxcard THEN c1.value := 0 ELSE INC(c1.value) END;
                  PutConst(atp,c1);
                END;
              ELSE
      ...
      CASE bptr^.klass OF
        funcs : (* function block *)
          ...
          ELSE
            ...
            at2.mode := varm;
(*
            IF NOT (ExpressionSystem.AssignComp(at2,at1) OR AddressComp(at1,at2)) THEN
              ErrorLS(155)
            END;
*)
            PutGetSy; (* rparent *)
    ...
    BEGIN
      ...
      LOOP
        ...
        (* IF NOT ExpressionSystem.AssignComp(at1,at2) THEN Error(128) END; *)
